...
peak_values_ant <- read_csv('../data/peak_cbv_differences_table_ketamine_anterior.csv')
...
peak_values_post <- read_csv('../data/peak_cbv_differences_table_ketamine_posterior.csv')
...
# One-way anova (sex)
res_aov_sex <- anova_test(
  data = peak_values, dv = Peak, wid = Animal_ID,
  between = Sex
)
res_aov_sex <- get_anova_table(res_aov_sex)

# write anova results to .txt file
write.csv(res_aov_sex, "res_peakdiff_oneway_anova_sex.csv", row.names=FALSE)
